mod tls_dh {
    use nom::sequence::pair;
    use tls_parser::*;

    #[rustfmt::skip]
static SERVER_ECDHE_PARAMS: &[u8] = &[
    
];

    #[test]
    fn test_tls_server_ecdhe_params() {
        let empty = &b""[..];
        let bytes = SERVER_ECDHE_PARAMS;
        let point_data = &bytes[4..137];
        let expected1 = ServerECDHParams {
            curve_params: ECParameters {
                curve_type: ECCurveType::NamedGroup,
                params_content: ECParametersContent::NamedGroup(NamedGroup::Secp521r1),
            },
            public: ECPoint { point: point_data },
        };
        let expected2 = DigitallySigned {
            alg: Some(SignatureAndHashAlgorithm {
                hash: HashAlgorithm::Sha1,
                sign: SignAlgorithm::Rsa,
            }),
            data: &bytes[141..],
        };
        let res = pair(parse_server_ecdh_params, parse_digitally_signed)(bytes);
        assert_eq!(res, Ok((empty, (expected1, expected2))));
    }

    #[rustfmt::skip]
    static CLIENT_ECDHE_PARAMS: &[u8] = &[
        0x61, 0x04, 0x6e, 0xe8, 0xe1, 0x0b, 0x00, 0x7d,
        0x73, 0xef, 0x1e, 0x73, 0x96, 0xcb, 0x68, 0xae,
        0x15, 0xe4, 0xa7, 0xca, 0x47, 0x17, 0xf6, 0x06,
        0x85, 0x4e, 0x05, 0x1f, 0x62, 0x0a, 0xcf, 0x56,
        0xe6, 0x5a, 0xd0, 0xde, 0x4d, 0xe7, 0x7e, 0x0f,
        0x66, 0x74, 0xc4, 0xff, 0xda, 0xae, 0x70, 0x70,
        0x09, 0x7b, 0x3c, 0x3f, 0x50, 0x8a, 0xe1, 0x0c,
        0x7b, 0x53, 0xe1, 0x24, 0x0e, 0x70, 0x6b, 0x66,
        0x93, 0xa6, 0x88, 0x4c, 0x8b, 0x58, 0x06, 0x52,
        0xc1, 0xf8, 0x50, 0x73, 0xbf, 0x87, 0xe7, 0x9a,
        0xa4, 0x50, 0x20, 0x91, 0x72, 0x36, 0xa8, 0xba,
        0x34, 0x7d, 0xb9, 0xda, 0xb6, 0xf8, 0xdd, 0xb3,
        0xca, 0x1b
    ];

    #[test]
    fn test_tls_client_ecdhe_params() {
        let empty = &b""[..];
        let bytes = CLIENT_ECDHE_PARAMS;
        let expected1 = ClientECDiffieHellmanPublic {
            ecdh_yc: ECPoint { point: &bytes[1..] },
        };
        let res = parse_client_ecdh_params(bytes);
        assert_eq!(res, Ok((empty, expected1)));
    }

    #[rustfmt::skip]
static SERVER_DHE_PARAMS: &[u8] = &[
    
];

    #[test]
    fn test_tls_server_dhe_params() {
        let empty = &b""[..];
        let bytes = SERVER_DHE_PARAMS;
        let expected1 = ServerDHParams {
            dh_p: &bytes[2..258],
            dh_g: &bytes[260..516],
            dh_ys: &bytes[518..774],
        };
        let expected2 = DigitallySigned {
            alg: Some(SignatureAndHashAlgorithm {
                hash: HashAlgorithm::Sha512,
                sign: SignAlgorithm::Rsa,
            }),
            data: &bytes[778..],
        };
        let res = pair(parse_server_dh_params, parse_digitally_signed)(bytes);
        assert_eq!(res, Ok((empty, (expected1, expected2))));
    }

    #[rustfmt::skip]
    static CLIENT_DHE_PARAMS: &[u8] = &[
        0x00, 0x40, 0xd6, 0x07, 0x6e, 0xcb, 0x9e, 0xc3,
        0x08, 0x3f, 0x7f, 0x5a, 0x35, 0xa4, 0x63, 0x88,
        0x3f, 0xba, 0x51, 0xd4, 0x4f, 0x8d, 0xc0, 0xdb,
        0x59, 0x6d, 0xd5, 0x05, 0xd3, 0x32, 0xe6, 0x24,
        0x3f, 0xec, 0x78, 0x9a, 0x21, 0xf3, 0xda, 0x4a,
        0x70, 0x8d, 0x6e, 0xb1, 0xca, 0xcf, 0x4b, 0xd0,
        0x90, 0x0b, 0xb3, 0x1b, 0x17, 0xba, 0xfc, 0x9c,
        0x8c, 0x00, 0xb3, 0x82, 0xf3, 0x6e, 0x9a, 0xad,
        0x15, 0xec
    ];

    #[test]
    fn test_tls_client_dhe_params() {
        let empty = &b""[..];
        let bytes = CLIENT_DHE_PARAMS;
        let expected1 = ClientDHPublic { dh_yc: &bytes[2..] };
        let res = parse_client_dh_params(bytes);
        assert_eq!(res, Ok((empty, expected1)));
    }
}
